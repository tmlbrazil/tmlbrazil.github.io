...
if (JSON.parse(localStorage.getItem('indiceAll')) > 0) {
  videos = JSON.parse(localStorage.getItem('notasSalvasAll'));
  indice = JSON.parse(localStorage.getItem('indiceAll'));
...
document.querySelector('#proximo').addEventListener('click', () => {
  ...
  atualizarTabela();
  indice++;
  localStorage.setItem('notasSalvasAll', JSON.stringify(videos));
  localStorage.setItem('indiceAll', JSON.stringify(indice));
  if (indice >= videos.length) {
    alert('Você avaliou todos os vídeos!');
    //
    indice = 0;
    JSON.parse(localStorage.getItem('indiceAll')) = indice ;
    //
    return;
  }
  videoInfo.textContent = `${videos[indice].nome} -${videos[indice].style}`
